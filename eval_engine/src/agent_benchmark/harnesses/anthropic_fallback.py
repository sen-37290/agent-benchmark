...
def _install_streaming_ledger() -> None:
    """Keep the ledger writing when the run streams.

    Harbor's LiteLLM backend can be told to stream (its ``stream`` option, which the Terminus 2
    adapter sets); it consumes the stream internally and returns one assembled response, so
    nothing above notices -- including this ledger, which is the problem.
    ...
    call too, so ``fallbacks`` and the beta header are already on the wire. The hook is harmless
    on a non-streaming run, where nothing ever calls it.
    """
